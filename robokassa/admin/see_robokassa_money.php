...
?>
<?php
	if( !check_right('RB_SEE_MON_IN',R_MSG)) return; 	//проверка прав
?>
...
<div align="center">
	<table class="yandex_money_table styled_table" width="100%">
		...
		$vars = array($table_robokassa);
        if(isset($user_id))
        	{$query .= " WHERE id_pokypatelya = '%d' "; $vars[] = $user_id;}
       	$res = $ROBOKASSA->db->query($query,$vars) or die($ROBOKASSA->db->error());
       	$row = $res->fetch_assoc();
       	$max = $row['count(id)'];
		if(empty($URL['PAGE']))
			$p=1;
	...
	<?php 	//вывод под таблицей ссылок перехода на страницы 'переидти'
		get_table_nav('ROBOKASSA', 'admin/see_robokassa_money', NULL, $max, $on_page); 
	?>
</div>
<style type="text/css">
